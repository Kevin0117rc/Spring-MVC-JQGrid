...
<script type="text/javascript">
	jq(function() {
		jq("#grid").jqGrid({
		   	url:'/spring-jqgrid-integration/kevin/crud',
			datatype: 'json',
			mtype: 'GET',
			colNames:['Id', 'First Name', 'Last Name','Address','City','State','Zip Code','Phone Number'],
		   	colModel:[
		   		{name:'id',index:'id', width:55,editable:false,editoptions:{readonly:true,size:10},hidden:true},
		   		{name:'firstName',index:'lastName', width:40,editable:true, editrules:{required:true}, editoptions:{size:10}},
		   		{name:'lastName',index:'firstName', width:40,editable:true, editrules:{required:true}, editoptions:{size:10}},
		   		...
		   		{name:'state',index:'state', width:20,editable:true, editrules:{required:true}, editoptions:{size:10}},
		   		{name:'zipPostal',index:'zipPostal', width:40,editable:true, editrules:{required:true}, editoptions:{size:10}},
		   		{name:'phoneNumber',index:'phoneNumber', width:40,editable:true, editrules:{required:true}, editoptions:{size:10}}		 
		   	],
		   	postData: { 
			},
			rowNum:20,
...
function addRow() {
    ...
    jq("#grid").jqGrid('editGridRow','new',
    		{ 	url: "/spring-jqgrid-integration/kevin/crud/add", 
					editData: {
			    },
			    recreateForm: true,
...
function editRow() {
	...
	if( row != null ) 
		jq("#grid").jqGrid('editGridRow',row,
			{	url: "/spring-jqgrid-integration/kevin/crud/edit", 
				editData: {
		        },
		        recreateForm: true,
		        beforeShowForm: function(form) {
...
function deleteRow() {
	...
	if( row != null ) 
		jq("#grid").jqGrid( 'delGridRow', row,
          	{ url: '/spring-jqgrid-integration/kevin/crud/delete', 
						recreateForm: true,
			            beforeShowForm: function(form) {
			              //change title
